...
jQuery(document).ready(function($){
    ...
    var popupSignUp = $('.popup-signup');
    if (popupSignUp) {
        popupSignUp.on('click', '.sign-up', function(e) {
            e.preventDefault();
            var redirect = $(this).attr('href');
            Cookies.set('csf-user-signed', 'true', { expires: 90 });
            popupSignUp.fadeOut('fast', function() {
                window.location.href = redirect;
            })
        });
        popupSignUp.on('click', '.sign-close', function(e) {
            e.preventDefault();
            popupSignUp.fadeOut(function() {
                popupSignUp.remove();
